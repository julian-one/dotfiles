-- lazydev attaches an LspAttach handler that extends lua_ls's workspace.library
-- when editing files under stdpath('config') or installed plugin paths.
...
-- mason-tool-installer drives installs; server attach is done by
-- vim.lsp.enable() below, so mason-lspconfig is unnecessary.
require("mason").setup({
	ui = {
		border = "rounded",
		icons = {
